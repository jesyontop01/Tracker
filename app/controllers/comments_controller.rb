class CommentsController < ApplicationController
 # before_action :authenticate_user!, except:[:index]
  ...
  def create
    @document = Document.find(params[:document_id])
    @comment =@document.comments.update(:status_flag => true)
    @comment =@document.comments.create(comment_params)

    respond_to do |format|
      if @comment.save
        format.html { redirect_to document_comments_path(@document), notice: 'Comment was successfully created.' }
        format.json { render :show, status: :created, location:document_comments_path(@document) }
      else
